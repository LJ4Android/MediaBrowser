...
/* add by linhuaji, to be done, 2014.07.28 */											
const char* didl_rtsp_protocolinfo = "\"http-get:*:video/mp4:DLNA.ORG_PN="
	                                    "MPEG4_P2_SP_AAC;DLNA.ORG_OP=01;DLNA.ORG_CI=0;"
	                                    "DLNA.ORG_FLAGS=01500000000000000000000000000000\"";
const char* didl_http_protocolinfo = "\"http-get:*:video/mp4:DLNA.ORG_PN="
	                                    "MPEG4_P2_SP_AAC;DLNA.ORG_OP=01;DLNA.ORG_CI=0;"
	                                    "DLNA.ORG_FLAGS=01500000000000000000000000000000\"";	 
/*end by linhuaji */
...
+---------------------------------------------------------------------*/
PLT_MicroMediaController::PLT_MicroMediaController(PLT_CtrlPointReference& ctrlPoint, DeviceCallback *callback, char*host) :
    PLT_SyncMediaBrowser(ctrlPoint, false, this), //linhuaji modify
    PLT_MediaController(ctrlPoint),
	PLT_FileTransmitController(ctrlPoint),
	mHost(host)
{
    /* added by lihaihua */
    mCallback = callback;
	...
	PLT_FileTransmitController::SetDelegate(this);
}
...
+---------------------------------------------------------------------*/
void PLT_MicroMediaController::OnRapidTransmitFileNameChange(const char*from,const char *filename)
{
    NPT_LOG_INFO_2("From %s Transmit File Name = %s", from, filename);
    mCallback->onRapidTransmitVideo(from, filename);
    return;
}
...
/*----------------------------------------------------------------------
|   PLT_MicroMediaController::OnFTDAdded
+---------------------------------------------------------------------*/
bool
PLT_MicroMediaController::OnFTDAdded(PLT_DeviceDataReference& device)
{
    NPT_String uuid = device->GetUUID();

    // test if it's a file transmit device
    PLT_Service* service;
    if (NPT_SUCCEEDED(device->FindServiceByType("urn:schemas-upnp-org:service:FileTransmitControl:*", service))) {
        NPT_AutoLock lock(m_FileTransmitDevices);
        m_FileTransmitDevices.Put(uuid, device);
    }
...
void 
PLT_MicroMediaController::OnFTDRemoved(PLT_DeviceDataReference& device ) 
{
	NPT_String uuid = device->GetUUID();

	{
		NPT_AutoLock lock(m_FileTransmitDevices);
		m_FileTransmitDevices.Erase(uuid);
	}

	{
		NPT_AutoLock lock(m_CurFileTransmitDeviceLock);

		// if it's the currently selected one, we have to get rid of it
		if (!m_CurFileTransmitDevice.IsNull() && m_CurFileTransmitDevice == device) {
			m_CurFileTransmitDevice = NULL;
		}
	}
...
/*----------------------------------------------------------------------
|   PLT_MicroMediaController::OnFTDStateVariablesChanged
+---------------------------------------------------------------------*/
void
PLT_MicroMediaController::OnFTDStateVariablesChanged(PLT_Service*                  service,
                                                    NPT_List<PLT_StateVariable*>* vars)
{
    NPT_String uuid = service->GetDevice()->GetUUID();

    PLT_DeviceDataReference device;
    GetCurFileTransmitDevice(device);
    if (device.IsNull() || (device->GetUUID().Compare(uuid, true) != 0))
        return;
...
/*----------------------------------------------------------------------
|   PLT_MicroMediaController::GetCurFileTransmitDevice
+---------------------------------------------------------------------*/
void
PLT_MicroMediaController::GetCurFileTransmitDevice(PLT_DeviceDataReference& ftdevic)
{
    NPT_AutoLock lock(m_CurFileTransmitDeviceLock);

    if (m_CurFileTransmitDevice.IsNull()) {
        printf("No File Transmit Device selected, select one with setFTD\n");
    } else {
        ftdevic = m_CurFileTransmitDevice;
    }
}
...
PLT_MicroMediaController::ScanDevice(long mx)
{
	...
	PLT_SyncMediaBrowser::m_CtrlPoint->Search(NPT_HttpUrl("239.255.255.250", 1900, "*"), "urn:schemas-upnp-org:device:FileTransmitControl:1", mx, 0.0);
...
{
    NPT_LOG_INFO("GetDmrList trying to get lock");
    NPT_AutoLock lock(m_CurMediaRendererLock);
    NPT_LOG_INFO("GetDmrList got lock");
    int count = 0;
    const NPT_List<PLT_DeviceMapEntry*>& entries = m_MediaRenderers.GetEntries();
    NPT_List<PLT_DeviceMapEntry*>::Iterator entry = entries.GetFirstItem();

    NPT_LOG_INFO("GetDmrList adding device to list");
    while (entry) {
        PLT_DeviceDataReference device = (*entry)->GetValue();
        NPT_String              name   = device->GetFriendlyName();

	NPT_LOG_INFO_3("The %d device: name: %s ,key: %s", count, (const char *)name, (const char *)(*entry)->GetKey());
        if(count < 10) {
            strcpy(list[count].name, (const char *)name);
            strcpy(list[count].uuid, (const char *)(*entry)->GetKey());
        }
...
/*----------------------------------------------------------------------
|   PLT_MicroMediaController::GetFtdList
+---------------------------------------------------------------------*/
int 
PLT_MicroMediaController::GetFtdList(DeviceInfo list[])
{
    NPT_AutoLock lock(m_CurFileTransmitDeviceLock);

    int count = 0;
    const NPT_List<PLT_DeviceMapEntry*>& entries = m_FileTransmitDevices.GetEntries();
    NPT_List<PLT_DeviceMapEntry*>::Iterator entry = entries.GetFirstItem();
...
PLT_MicroMediaController::SetDms(const char *dmsname)
{
    ...
    while (entry) {
        ...
        if(strncmp(name, dmsname, strlen(name)) == 0) {
...
PLT_MicroMediaController::SetDmr(const char *dmrname)
{
    ...
    while (entry) {
        PLT_DeviceDataReference device = (*entry)->GetValue();
        NPT_String              name   = device->GetFriendlyName();
        if(strncmp(name, dmrname, strlen(name)) == 0) {
...
/*----------------------------------------------------------------------
|   PLT_MicroMediaController::SetFtd
+---------------------------------------------------------------------*/
int 
PLT_MicroMediaController::SetFtd(const char *ftdname)
{
    NPT_AutoLock lock(m_CurFileTransmitDeviceLock);
    ...
    while (entry) {
        PLT_DeviceDataReference device = (*entry)->GetValue();
        NPT_String              name   = device->GetFriendlyName();
	 NPT_LOG_INFO_2("ftd device %s (%s)", (const char *)name, (const char *)(*entry)->GetKey());
        if(strncmp(name, ftdname, strlen(name)) == 0) {
            NPT_LOG_INFO_2("set ftd to %s (%s)", (const char *)name, (const char *)(*entry)->GetKey());
            m_CurFileTransmitDevice = device;
            return 0;
        }
        ++entry;
    }
...
| PLT_MicroMediaController::StartTransmitFile
+---------------------------------------------------------------------*/
void
PLT_MicroMediaController::StartTransmitFile(char* sessionid, char* filename, char* size)
{
    PLT_DeviceDataReference device;
    GetCurFileTransmitDevice(device);

    if (!device.IsNull()) {
	NPT_LOG_INFO(" PLT_MicroMediaController StartTransmitFile");
        startTransmitFile(device, 0, "Master", sessionid, filename, size, GetHostName(), NULL);
    } else {
        NPT_LOG_INFO(" What happen!!!!!!!!!!");
    }
}

void PLT_MicroMediaController::OnStartTransmitFileResult(
        NPT_Result               res, 
        PLT_DeviceDataReference&  device ,
        const char*          sid , 
        const char*          filename , 
...
{
    if(res == 0){
	//char* from = this->GetHostName();
	PLT_DeviceDataReference dev;
    	GetCurFileTransmitDevice(dev);
	if(!dev.IsNull()){
	    char* from = dev->GetFriendlyName();
	    NPT_LOG_INFO_3("OnStartTransmitFileResult NPT_SUCCESS sid: %s, from: %s, filename: %s", sid, from, filename);
	    mCallback->onTransmitFileSessionControl(res, FILE_TRANSMIT_DEVICE_TOCONTRPOINT_SET_TRANSMIT_FILE, sid, from, filename);    
	    return;
	} 
    } else {
        NPT_LOG_INFO_1("OnStartTransmitFileResult Failed %s", sid);
        mCallback->onTransmitFileSessionControl(res, FILE_TRANSMIT_DEVICE_TOCONTRPOINT_SET_TRANSMIT_FILE, sid, "", ""); 
    }    
}

/*----------------------------------------------------------------------
| add by linhuaji 2014.06.26
| PLT_MicroMediaController::CancelTransmitFile
+---------------------------------------------------------------------*/
void
PLT_MicroMediaController::CancelTransmitFile(char* sessionid)
{
    PLT_DeviceDataReference device;
    GetCurFileTransmitDevice(device);

    if (!device.IsNull()) {
        cancelTransmitFile(device, 0, "Master", sessionid, NULL);
    }
}

void PLT_MicroMediaController::OnCancelTransmitFileResult(
        NPT_Result               res , 
        PLT_DeviceDataReference&  device ,
        const char*   sid ,
        void*                     userdata ) 
{
    if(res == 0){
	NPT_LOG_INFO_1("OnCancelTransmitFileResult NPT_SUCCESS %s", sid);
	mCallback->onTransmitFileSessionControl(res, FILE_TRANSMIT_DEVICE_TOCONTRPOINT_SET_CANCEL, sid, "", "");        
    } else {
	NPT_LOG_INFO_1("OnCancelTransmitFileResult Failed %s", sid);
        mCallback->onTransmitFileSessionControl(res, FILE_TRANSMIT_DEVICE_TOCONTRPOINT_SET_CANCEL, sid, "", "");
    }
}
...
/*----------------------------------------------------------------------
| add by linhuaji 2014.06.26
| PLT_MicroMediaController::FinishTransmitFile
+---------------------------------------------------------------------*/
void
PLT_MicroMediaController::FinishTransmitFile(char* sessionid)
{
    PLT_DeviceDataReference device;
    GetCurFileTransmitDevice(device);

    if (!device.IsNull()) {
        finishTransmitFile(device, 0, "Master", sessionid, NULL);
    } else {
	NPT_LOG_INFO("FinishTransmitFile Failed! Device not found!");
    }
}

void PLT_MicroMediaController::OnFinishTransmitFileResult(
        NPT_Result                res ,
        PLT_DeviceDataReference&  device ,
        const char*            sid ,
        void*                     userdata ) 
{
    if(res == 0){
	NPT_LOG_INFO_1("OnFinishTransmitFileResult NPT_SUCCESS %s", sid);
	mCallback->onTransmitFileSessionControl(res, FILE_TRANSMIT_DEVICE_TOCONTRPOINT_SET_FINISH, sid, "","");        
    } else {
	NPT_LOG_INFO_1("OnFinishTransmitFileResult Failed %s", sid);
        mCallback->onTransmitFileSessionControl(res, FILE_TRANSMIT_DEVICE_TOCONTRPOINT_SET_CANCEL, sid, "", "");
    }
}
...
/*----------------------------------------------------------------------
| add by linhuaji 2014.06.26
| PLT_MicroMediaController::GetTransmitFileProgress
+---------------------------------------------------------------------*/
void
PLT_MicroMediaController::GetTransmitFileProgress(char* sessionid)
{
    PLT_DeviceDataReference device;
    GetCurFileTransmitDevice(device);

    if (!device.IsNull()) {
	NPT_LOG_INFO_1("getTransmitFileProgress SID %s", sessionid);
        getTransmitFileProgress(device, 0, "Master", sessionid, NULL);
    }
}

void PLT_MicroMediaController::OnGetTransmitFileProgressResult(
        NPT_Result               res ,
        PLT_DeviceDataReference& device ,
        const char*            sid ,
        const char*            progress ,
 ...
 {
    if(res == 0){
	NPT_LOG_INFO_1("OnGetTransmitFileProgressResult NPT_SUCCESS %s", sid);
	mCallback->onTransmitFileSessionControl(res, FILE_TRANSMIT_DEVICE_TOCONTRPOINT_SET_PROGRESS, sid, "", progress);        
    } else {
	NPT_LOG_INFO_1("OnGetTransmitFileProgressResult Failed %s", sid);
        mCallback->onTransmitFileSessionControl(res, FILE_TRANSMIT_DEVICE_TOCONTRPOINT_SET_PROGRESS, sid, "", ""); 
    }
}
...
PLT_MicroMediaController::GetDownloadUri(const char *uri, char *dluri, int flags)
{
    ...
    if (!m_MostRecentBrowseResults.IsNull()) {
        ...
        while (item) {
            if (!(*item)->IsContainer()) {
                NPT_LOG_INFO_2("find file %s list file: %s", uri, (*item)->m_ObjectID.GetChars());
                if(strncmp(uri, (*item)->m_ObjectID.GetChars(), strlen(uri)) == 0) {
...
/* open */
	...
	NPT_LOG_INFO_1("CtrlPlay2 Render Device found: %s", (char*)device->GetFriendlyName());
        ...
        if (!m_MostRecentBrowseResults.IsNull()) {
            ...
            while (item) {
                if (!(*item)->IsContainer()) {
                    if(strncmp(uri, (*item)->m_ObjectID.GetChars(), strlen((*item)->m_ObjectID.GetChars())) == 0) {
	                NPT_LOG_INFO_1("Find file: %s", uri);
                        break;
                    }
            ...
            if ((*item)->m_ObjectID.GetLength()) {
                ...
                if (NPT_SUCCEEDED(NPT_ContainerFind(*m_MostRecentBrowseResults, PLT_MediaItemIDFinder((*item)->m_ObjectID), track))) {
                    if (track->m_Resources.GetItemCount() > 0) {
                        ...
                        if(strncmp(track->m_ObjectClass.type.GetChars(), "object.item", strlen("object.item")) == 0) {
                        ...
                        m_MostRecentBrowseResults = NULL;
                        /* add for debug, by linhuaji, 2014.08.14 */
                        NPT_LOG_INFO_1("Device friendly name: %s", (char*)device->GetFriendlyName());
                        NPT_LOG_INFO_1("Device local ip: %s", (char*)device->GetLocalIP().ToString());
                        /* end add */
...
/* open */
    ...
    if (!device.IsNull()) {
        ...
        if (!m_MostRecentBrowseResults.IsNull()) {
            ...
            while (item) {
                if (!(*item)->IsContainer()) {
                    if(strncmp(filename, (*item)->m_ObjectID.GetChars(), strlen((*item)->m_ObjectID.GetChars())) == 0) {
	                NPT_LOG_INFO_1("Find file: %s", uri);
                        break;
                    }
                }
                ...
                if (NPT_SUCCEEDED(NPT_ContainerFind(*m_MostRecentBrowseResults, PLT_MediaItemIDFinder((*item)->m_ObjectID), track))) {
                    if (track->m_Resources.GetItemCount() > 0) {
                        ...
                        NPT_LOG_INFO_2("Issuing SetAVTransportURI with url=%s & didl=%s", 
                            ...
                            (const char*)track->m_Didl);
                        /* qiku modify */
						/* When there are multi network interface, we may chose the
						 * wrong one(the one the client unreachable), we try to fix the
						 * host name here, Note: these is just for the local DMS device, for
						 * other DMS, this may cause problem. */
                        NPT_HttpUrl http_url(track->m_Resources[resource_index].m_Uri);
                        if (!http_url.IsValid()) {
                             NPT_LOG_FATAL_1("Invalid uri: %s", (const char*)track->m_Resources[resource_index].m_Uri);
                             return -1;
                        }
                        http_url.SetHost((char*)device->GetLocalIP().ToString());
                        NPT_LOG_INFO_1("PlayUri(fixed) : %s", (char*)http_url.ToString(true));
                        SetAVTransportURI(device, 0, http_url.ToString(true), track->m_Didl, NULL);
						/* qiku end */
                        m_MostRecentBrowseResults = NULL;
    	...
    	NPT_LOG_WARNING("GetCurMediaRenderer failed!");
    }
...
/* open */
    ...
    if (!device.IsNull()) {
        ...
        snprintf(didl, 1024, "%s<item id=\"0/1\" parentID=\"0\" restricted=\"1\"><dc:title>%s</dc:title>"
                                      "<upnp:class>%s</upnp:class><res protocolInfo=%s>%s</res></item>%s", 
                                    didl_header, title, upnpClass, didl_http_protocolinfo, uri, didl_footer);
