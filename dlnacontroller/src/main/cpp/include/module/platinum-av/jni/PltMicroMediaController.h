...
public:
    ...
    virtual int onRapidTransmitVideo(const char *from, const char *filename){return 0;}
    virtual int onTransmitFileSessionControl(int res, int cmd, const char *sid, const char *from, const char *data) { return 0; }
	/* end by linhuaji */
private:
    uint32_t mTest;
};
...
class PLT_MicroMediaController : public PLT_SyncMediaBrowser,
                                 ...
                                 public PLT_FileTransmitController,	//add by linhuaji
                                 public PLT_FileTransmitControllerDelegate,
                                 public PLT_MediaContainerChangesListener
{
public:
    PLT_MicroMediaController(PLT_CtrlPointReference& ctrlPoint, DeviceCallback *callback, char *host);
   ...
   //PLT_FileTransmitControllerDelegate methods
   bool OnFTDAdded(PLT_DeviceDataReference& /* device */);
    void OnFTDRemoved(PLT_DeviceDataReference& /* device */) ;
    void OnFTDStateVariablesChanged(PLT_Service*                  /* service */, 
                                           NPT_List<PLT_StateVariable*>* /* vars */) ;
    void OnStartTransmitFileResult(NPT_Result res,
    		PLT_DeviceDataReference& device,
    		const char*           sid, 
    		const char*           filename, 
    		void*                     userdata);

    void OnCancelTransmitFileResult(NPT_Result  res,
		PLT_DeviceDataReference& device,
		const char*  sid,
		void*                    userdata);

    void OnFinishTransmitFileResult(NPT_Result res, 
		PLT_DeviceDataReference&  device,
		const char*            sid,
		void*                     userdata) ;

    void OnGetTransmitFileProgressResult(NPT_Result res ,
        PLT_DeviceDataReference&  device ,
        const char*            sid ,
        const char*            progress ,
        void*                     userdata );
    ...
    void OnRapidTransmitFileNameChange(const char *from, const char *filename);
    /* qiku end */
...
/* added by lihaihua */
    ...
    int GetFtdList(DeviceInfo list[]);
    int SetDms(const char *dmsname);
    int SetDmr(const char *dmrname);
    int SetFtd(const char *ftdname);
    ...
    void StartTransmitFile(char* sessionid, char* filename, char* size);
    void CancelTransmitFile(char* sessionid);
    void FinishTransmitFile(char* sessionid);
    void GetTransmitFileProgress(char* sessionid);
...
private:
    ...
    void        GetCurFileTransmitDevice(PLT_DeviceDataReference& ftdevic);
...
private:
    ...
    //qiku add
    NPT_Lock<PLT_DeviceMap> m_FileTransmitDevices;
    ...
    //qiku add
    PLT_DeviceDataReference m_CurFileTransmitDevice;
    NPT_Mutex               m_CurFileTransmitDeviceLock;
