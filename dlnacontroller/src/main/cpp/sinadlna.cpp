...
int SinaDLNA::startMediaServer(const char *path, const char *friendly_name) {
	LOGI("SinaDLNA::startMediaServer");
	this->mediaServer=new PLT_MicroMediaServer();
	this->mediaServer->startMediaServer(path,friendly_name);
	return 0;
}

int SinaDLNA::stopMediaServer() {
	if(NULL == this->mediaServer)
		return -1;
	this->mediaServer->stopMediaServer();
	return 0;
}

int SinaDLNA::startMediaRender(const char *friendly_name) {
	this->mediaRenderer=new PLT_MicroMediaRenderer();
	LOGI("SinaDLNA::startMediaRender");
	this->mediaRenderer->startMediaRenderer(friendly_name);
	return 0;
}

int SinaDLNA::stopMediaRender() {
	if(NULL == this->mediaRenderer)
		return -1;
	this->mediaRenderer->stopMediaRenderer();
	return 0;
}
