...
static const char* const kClassPathName 	= 	"com/lj/dlnacontroller/api/SinaDLNA";
...
static int startMediaServer(JNIEnv * env, jobject thiz,jstring path,jstring friendly_name){
    LOGI("jni->startMediaServer");
    int ret;
    SinaDLNA* dlna = getSinaDLNA(env, thiz);
    if (dlna == NULL ) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
    ...
    if (pth == NULL) {  // Out of memory
        jniThrowException(env, "java/lang/RuntimeException", "Out of memory");
        return SinaDLNA::SINA_DLNA_CMD_RESULT_FAIL;
    }
    LOGI("startMediaServer");
    ret = dlna->startMediaServer(pth,name);
...
static int stopMediaServer(JNIEnv * env, jobject thiz){
    ...
    dlna->stopMediaServer();
    return 0;
}

static int startMediaRender(JNIEnv * env, jobject thiz, jstring friendly_name){
    int ret;
    SinaDLNA* dlna = getSinaDLNA(env, thiz);
    if (dlna == NULL ) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
    ...
    const char *tmp = env->GetStringUTFChars(friendly_name, NULL);
    if (tmp == NULL) {  // Out of memory
        jniThrowException(env, "java/lang/RuntimeException", "Out of memory");
        return SinaDLNA::SINA_DLNA_CMD_RESULT_FAIL;
    }
    LOGI("startMediaRender");
    ret = dlna->startMediaRender(tmp);

    env->ReleaseStringUTFChars(friendly_name, tmp);
    tmp = NULL;
    return ret;
}
...
//	{"end",        			"()V",            								(void *)end},
    ...
    {"startMediaServer",  "(Ljava/lang/String;Ljava/lang/String;)I",  (void *)startMediaServer},
    {"stopMediaRender",    "()I",            						        (void *)stopMediaRender},
    {"startMediaRender",  "(Ljava/lang/String;)I",                      (void *)startMediaRender},
