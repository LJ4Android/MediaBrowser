...
public class MediaLibrary {
    ...
    public ArrayList<MediaWrapper> getVideoItems() {
        ...
        Log.d(TAG,"getVideoItems "+videoItems.size());
        return videoItems;
    }
    ...
    private class GetMediaItemsRunnable implements Runnable {
        ...
        public void run() {
            ...
            Log.d(TAG,"GetMediaItemsRunnable_mediaDirs:"+mediaDirs.size());
            if (mediaDirs.size() == 0) {
            ...
            try {
                ...
                while (!directories.isEmpty()) {
                    ...
                    try {
                        ...
                        Log.d(TAG,"CanonicalPath:"+dirPath);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                ...
                for (File file : mediaToScan) {
                    ...
                    if (existingMedias.containsKey(fileURI)) {
                        ...
                        MediaWrapper mw = new MediaWrapper(Uri.fromFile(file));
                        mw.setLastModified(file.lastModified());
                        mItemListLock.writeLock().lock();
                        Log.d(TAG,"not_containsKey:"+fileURI);
                        mItemList.add(mw);
                        mItemListLock.writeLock().unlock();
